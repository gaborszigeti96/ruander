function MegtettUt(sebesseg: number, ido: number): number {
    let megtettUt: number = sebesseg * ido;
    return megtettUt
}
...
function LeetKod(vizsgaltSzoveg: string): string {
    let szoveg: string = vizsgaltSzoveg.replace(/i/g, "1");
    let szoveg1: string = szoveg.replace(/I/g, "1")
    let szoveg2: string = szoveg1.replace(/o/g, "0")
    let szoveg3: string = szoveg2.replace(/O/g, "0")
    let szoveg4: string = szoveg3.replace(/a/g, "4")
    let szoveg5: string = szoveg4.replace(/A/g, "4")
    let szoveg6: string = szoveg5.replace(/e/g, "3")
    let szoveg7: string = szoveg6.replace(/E/g, "3")
    /*
    for (let i: number = 0; i < szoveg.length; i++) {
        if (szoveg[i] == "i" || szoveg[i] == "I") {
            szoveg[i].replace("1")
        }
    }
    */
...
function LegtobbNyeremeny(vizsgaltTomb: snooker[]): number {
    let legtobbpont: number = 0;
    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
        if (vizsgaltTomb[i].eredmeny > legtobbpont) {
            legtobbpont = vizsgaltTomb[i].eredmeny
        }
